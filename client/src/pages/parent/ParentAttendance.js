...
export default function ParentAttendance() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/attendance/my`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => { setRecords(res.data.records); setSubjectStats(res.data.subjectStats); setLoading(false); })
  ...
  }).forEach(r => { dateStatusMap[new Date(r.date).getDate()] = r.status; });
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f7fafc" }}>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"2rem" }}>
        <h2 style={{ fontSize:"1.6rem", fontWeight:700, color:"#1a202c", marginBottom:"1.5rem" }}>📅 Child's Attendance</h2>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))", gap:"1rem", marginBottom:"2rem" }}>
            {subjectStats.map(s => (
              <div key={s.subject} style={{ background:"#fff", borderRadius:12, padding:"1rem", boxShadow:"0 1px 6px rgba(0,0,0,0.08)" }}>
                <div style={{ fontWeight:600, color:"#2d3748", marginBottom:4 }}>{s.subject}</div>
                <div style={{ fontSize:"2rem", fontWeight:700, color:s.percentage>=75?"#22c55e":"#ef4444" }}>{s.percentage}%</div>
                <div style={{ fontSize:"0.8rem", color:"#888" }}>{s.present}/{s.total} classes</div>
                <div style={{ marginTop:6, height:5, borderRadius:99, background:"#f0f0f0" }}>
        ...
        <div style={{ background:"#fff", borderRadius:12, padding:"1.5rem", boxShadow:"0 1px 6px rgba(0,0,0,0.08)" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:4, textAlign:"center" }}>
            {DAYS.map(d => <div key={d} style={{ fontWeight:600, fontSize:"0.75rem", color:"#888", paddingBottom:4 }}>{d}</div>)}
            {Array(firstDay).fill(null).map((_,i) => <div key={`e${i}`} />)}
            {Array(daysInMonth).fill(null).map((_,i) => {
              const day = i+1; const status = dateStatusMap[day];
              return (
                <div key={day} style={{ padding:"0.4rem 0", borderRadius:8, fontSize:"0.85rem",
          ...
          <div style={{ display:"flex", gap:"1rem", marginTop:"1rem", fontSize:"0.8rem" }}>
            {Object.entries(statusColor).map(([st,c]) => (
...
const s = { navBtn: { border:"1px solid #e2e8f0", background:"#fff", borderRadius:8, padding:"0.3rem 0.8rem", cursor:"pointer" } };